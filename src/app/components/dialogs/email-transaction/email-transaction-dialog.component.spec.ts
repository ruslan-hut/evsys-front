...
import { EmailTransactionDialogComponent, EmailTransactionDialogData } from './email-transaction-dialog.component';
...
describe('EmailTransactionDialogComponent', () => {
  ...
  let dialogRef: MockedObject<Pick<MatDialogRef<EmailTransactionDialogComponent>, 'close'>>;
  ...
  it('rejects addresses the backend would reject', () => {
    ...
    for (const bad of ['not-an-email', 'a@b', 'a b@example.com', 'a@b.c']) {
      ...
      expect(component.email.valid, `"${bad}" must be invalid`).toBe(false);
    }
  });
  ...
  it('disables the send button until the address is valid', () => {
    setup({ transactionId: 42 });
    const sendButton = (): HTMLButtonElement => fixture.nativeElement.querySelectorAll('button')[1];
